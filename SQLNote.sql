...
二、	sql语句
	...
	3.	数据类型：
		char(n)：定长字符串，n是字节数。
		varchar2(n):变长字符串，n是字节数。最多2000字节。
		number(n,m):n共多少位数字，m是其中几位小数。最多4000字节。
...
10.常用函数(单行函数)
	...
	日期常用函数：
		(1)LAST_DATE(date):返回指定日期所在月的最后一天。
		(2)ADD_MONTHS(date,i):返回日期date加上i个月后的日期值，若i是负数，则获得的是减去i个月后的日期值。
		(3)MONTHS_BETWEEN(date1,date2):计算两个日期之间相差多少个月;若date2时间比date1时间晚，会得到负值，除非两个日期间隔是整数月，否则会得到带小数位的结果。
		(4)date日期类型在数据库中可以进行减法操作，结果是相差的天数：select ename, sysdate-hiredate from emp;
		(5)LEAST(date1,date2):显示两个时间中的最小值。参数类型必须一致。
		(6)GREATEST(date1,date2):显示两个时间中的最大值。参数类型必须一致。
		(7)EXTRACT(date from datetime):从参数datetime中提取参数date指定的数据，比如提取：年、月、日。
	(4)空值函数：
...
15.聚合函数(多行函数，分组函数)
	(1)MAX和MIN:用来取得列或表达式的最大、最小值。可以用来统计任何数据类型，包括数字、字符和日期。
...
18.函数总结
	单行函数和多行函数的分类并举例说明：
		单行函数：
			字符函数：CONCAT(c1,c2), LENGTH(ch), UPPER(字段名), LOWER(字段名), INITCAP(字段名), TRIM('e'FROM'ee'), LTRIM('ee','e'), RTRIM('ee','e'), LPAD(ch1, m, ch2), RPAD(ch1, m, ch2), SUBSTR（ch,m,n), INSTR(ch1,ch2,m,n)
			数值函数：ROUND(n,m), TRUNC(n,m), MOD(m,n), CEIL(n), FLOOR(n)
			日期函数：sysdate, systimestamp, LAST_DATE(date), ADD_MONTHS(date,i), MONTHS_BETWEEN(date1,date2), LEAST(date1,date2), GREATEST(date1,date2), EXTRACT(date from datetime)
			转换函数：
...
22.分页查询：
	...
	ROWNUM:伪列，系统自动生成的一列，用来表示行号;
		...
		/*工资降序后选6~10条记录：原表先降序，再加伪列，再当新表选范围*/select * from (select rownum rn, e.* from (select * from emp order by sal desc) e) t where t.rn between 6 and 10;
...
26.列的约束：约束主要是用来约束表中数据的规则。
	...
	非空约束：not null 
	唯一约束：unique
	检查约束：check(条件) 在MySql中是可以写的，但MySql会直接忽略检查
